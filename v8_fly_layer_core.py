...
def z_momentum(closes,minutes=5,vol_window=30):
    ...
    sigma=statistics.pstdev(logs[-vol_window:]); return 0.0 if sigma<=1e-12 else math.log(closes[-1]/closes[-1-minutes])/(sigma*math.sqrt(minutes))
...
async def strategy(symbol):
    ...
    h=[float(x[2]) for x in a1]; l=[float(x[3]) for x in a1]; c=[float(x[4]) for x in a1]; v=[float(x[5]) for x in a1]; h5=[float(x[2]) for x in a5]; l5=[float(x[3]) for x in a5]; c5=[float(x[4]) for x in a5]
    ct=int(a1[-1][0]); hi,lo,cl=h[-1],l[-1],c[-1]; e9,e21=m.ema(c,9),m.ema(c,21); e20,e50=m.ema(c5,20),m.ema(c5,50); rv=m.rsi_wilder(c); av=m.atr_wilder(h,l,c); ad=m.adx_wilder(h5,l5,c5); mh,mhp=m.macd_hist(c); vw=m.vwap(h,l,c,v)
...
async def manage_position():
    ...
    if age<due:return
    a=await strategy(p['symbol']); z=float(a.get('z_momentum') or 0); imb=float(a.get('book_imbalance') or .5); spread=float(a.get('real_spread_pct') or 1); trigger=float(p.get('entry_trigger') or e)
    if p['side']=='LONG': trend_ok=a.get('regime')=='TREND_LONG'; momentum_ok=z>=0.15; book_ok=imb>=m.BOOK_LONG_MIN; structure_ok=price>=trigger-d*.15
    else: trend_ok=a.get('regime')=='TREND_SHORT'; momentum_ok=z<=-0.15; book_ok=imb<=m.BOOK_SHORT_MAX; structure_ok=price<=trigger+d*.15
    healthy=sum([trend_ok,momentum_ok,book_ok,structure_ok,spread<=MAX_REAL_SPREAD_PCT]); extensions=int(p.get('time_extensions',0))
    if healthy>=3 and extensions<SMART_TIME_MAX_EXTENSIONS:
        p['time_extensions']=extensions+1;p['next_time_check_min']=age+SMART_TIME_GRACE_MINUTES;p['last_time_health']=healthy;m.save_state();return
    close_trade(price,f'SMART TIME EXIT health={healthy}/5 ext={extensions}')
...
async def cycle():
    try:
        ...
        if best.get('signal') in ('LONG','SHORT'):open_trade(best,price);return
        ...
        tol=atr*RETEST_TOL_ATR; touched=trigger-tol<=price<=trigger+tol
