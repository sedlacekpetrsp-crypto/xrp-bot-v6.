...
@app.get("/health")
async def health():
    return {"ok":True,"bot":"V9 BEST-OF","mode":TRADING_MODE,"running":bot_task is not None and not bot_task.done(),"last_cycle_at":last_cycle_at,"last_error":last_error}
...
async function go(){try{const d=await (await fetch('/analyze',{cache:'no-store'})).json();let h=`<div class='grid'><div class='card'><div class='muted'>Balance</div><div class='big'>${f(d.paper_balance)} USDT</div></div><div class='card'><div class='muted'>Equity</div><div class='big ${cls(d.equity-d.paper_balance)}'>${f(d.equity)} USDT</div></div><div class='card'><div class='muted'>Uzavřené obchody</div><div class='big'>${d.stats.count}</div></div><div class='card'><div class='muted'>Win rate</div><div class='big'>${f(d.stats.win_rate,1)} %</div></div><div class='card'><div class='muted'>P/L</div><div class='big ${cls(d.stats.pnl)}'>${f(d.stats.pnl)} USDT</div></div><div class='card'><div class='muted'>Risk / NET R:R</div><div class='big'>${f(d.risk_per_trade_pct,2)}% · 1:${f(d.net_rr,2)}</div></div></div>`;
h+=`<div class='grid'>`+Object.values(d.symbols||{}).map(x=>{const s=x.signal||'WAIT';return `<div class='card'><b>${x.symbol}</b><div class='big ${s==='LONG'?'green':s==='SHORT'?'red':'yellow'}'>${s}</div><div class='row'><span>Setup</span><b>${x.setup||'—'}</b></div><div class='row'><span>1h trend</span><b>${x.trend_1h||'—'}</b></div><div class='row'><span>15m trend</span><b>${x.trend_15m||'—'}</b></div><div class='row'><span>Volume</span><b>${f(x.volume_ratio,2)}×</b></div><small class='muted'>${x.reason||''}</small></div>`}).join('')+`</div>`;
h+=`<div class='card'><h3>Historie</h3><div class='scroll'><table><tr><th>Pár</th><th>Směr</th><th>Setup</th><th>P/L</th><th>Důvod</th></tr>`+(d.trade_history||[]).map(t=>`<tr><td>${t.symbol}</td><td class='${t.side==='LONG'?'green':'red'}'>${t.side}</td><td>${t.setup}</td><td class='${cls(t.pnl)}'>${f(t.pnl)}</td><td>${t.reason}</td></tr>`).join('')+`</table></div></div>`;root.innerHTML=h}catch(e){root.innerHTML='<div class=card>Chyba načtení</div>'}}go();setInterval(go,15000);
</script></body></html>""")
