...
.section-head{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px}.section-head h2{margin:0}.count-badge{font-size:13px;font-weight:800;color:#21d19f;background:#10372f;border:1px solid #1b5c4d;border-radius:999px;padding:6px 10px;white-space:nowrap}
...
<div class="grid"><div class="card"><div class="section-head"><h2>V8 Candle Scanner</h2><span id="scannerCount" class="count-badge">— obchodů</span></div><div id="scanner">Načítám…</div><p id="scannerReason" class="wait"></p><div id="scannerStats"></div></div><div class="card"><div class="section-head"><h2>V8 Candle Fixed</h2><span id="fixedCount" class="count-badge">— obchodů</span></div><div id="fixed">Načítám…</div><p id="fixedReason" class="wait"></p><div id="fixedStats"></div></div></div>
...
function renderStats(data,el,name){
 ...
 el.innerHTML=`<div class="stats-box"><div class="stats-title">${name.toUpperCase()} · VYHODNOCENÍ UZAVŘENÝCH OBCHODŮ</div><div class="stats-grid"><div class="stats-cell"><div class="stats-value ${winrate>=50?'ok':'red'}">${historyNumber(winrate,1)} %</div><div class="stats-label">Winrate</div></div><div class="stats-cell"><div class="stats-value">${wins.length} / ${losses.length}</div><div class="stats-label">Wins / Losses</div></div><div class="stats-cell"><div class="stats-value ${profitFactor>=1?'ok':'red'}">${Number.isFinite(profitFactor)?historyNumber(profitFactor,2):'∞'}</div><div class="stats-label">Profit factor</div></div><div class="stats-cell"><div class="stats-value ${net>=0?'ok':'red'}">${historyNumber(net,2)} USDT</div><div class="stats-label">Čistý P/L</div></div></div><div class="stats-note">Uzavřené obchody ${name}: ${closed.length} · Uložená historie má nejvýše 100 obchodů.</div></div>`;
}
