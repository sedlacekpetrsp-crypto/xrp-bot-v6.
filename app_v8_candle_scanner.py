...
BUILD = "combined-v8-account-stats-adaptive-risk-history-fix-2-pnl-breakdown"
...
function positionsHtml(d){const ps=d.positions||[];if(!ps.length)return '<div class="muted">Bez otevřené pozice — Balance a Equity jsou nyní stejné.</div>';return ps.map(p=>{const qty=Number(p.qty||0),entry=Number(p.entry_price||0),now=Number(p.current_price||0),gross=(p.side==='SHORT'?(entry-now):(now-entry))*qty,net=Number(p.unrealized_pnl||0),costs=gross-net;return `<div class="pos"><div class="row"><div><span class="coin">${p.symbol}</span> <b class="${p.side==='LONG'?'ok':'red'}">${p.side}</b> · ${String(p.setup||'').replaceAll('_',' ')}</div><div><div class="${cls(net)}"><b>Čistý ${net>=0?'+':''}${nf(net)} USDT</b></div><div class="muted">Hrubý <span class="${cls(gross)}">${gross>=0?'+':''}${nf(gross)} USDT</span> · odhad nákladů ${nf(Math.max(costs,0))} USDT</div></div></div><div class="muted">Entry ${nf(p.entry_price,6)} · Now ${nf(p.current_price,6)} · SL ${nf(p.stop_loss,6)} · TP ${nf(p.take_profit,6)} · risk guard ×${nf(p.adaptive_risk?.multiplier??1,2)}</div></div>`}).join('')}
